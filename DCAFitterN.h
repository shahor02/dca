...
#ifndef _ALICEO2_DCAN_FITTER_
#define _ALICEO2_DCAN_FITTER_
...
class DCAFitter
{
  static constexpr int MaxProngs = 4;     //! max prongs considered
  static constexpt int MaxCand = 4;       //! max vertex candidates 
...
#define propagateParamTo PropagateParamOnlyTo
  ...
  struct TrackDeriv2 {
    ...
    // calculate sum_i^3{ v[i]*dTracksXYZ/dX } 
    auto dotProdD1(const Triplet& v) const { return v.x + v.y*dydx + v.z*dzdx; }

    // return 1st derivatives vector rotated by precalculated rotation matrix
    Triplet rotateD1(dtype_t s, dtype c) { return Triplet(c+s*dydx, -s+cos*dydx, dzdx); }
  };
  ...
  //----------------------------------------------------
  ///< point in space (vertex, point on track trajectory)
  struct Triplet {
    union {
      struct {
	ftype_t x, y, z;
  ...
  struct Ch2iDerivatives {
    std::array<ftype_t,MaxProngs> d1;                  // 1st derivatives of chi2 vs tracks local parameters X
    std::array<ftype_t,MaxProngs*MaxProngs> d2;        // 2nd derivatives (symmetric matrix, only upper triangle is used)

    auto  der1(int i) const {return d1[i];}
    auto& der1(int i)       {return d1[i];}
    auto  der2(int i,int j) const {return d2[i<j ? i*MaxProngs+j : j*MaxProngs+i];}
    auto  der2(int i,int j)       {return d2[i<j ? i*MaxProngs+j : j*MaxProngs+i];}
  };

  // <--- Auxiliary structs used by DCA finder
  using ArrTrack     = std::array<Track,MaxProngs>;              //! container for prongs (tracks) at single vertex cand. 
  using ArrTrackCovI = std::array<TrackCovI,MaxProngs>;          //! container for inv.cov.matrices at single vertex cand. 
  using ArrTrCoef    = std::array<TrackCoefVtx,MaxProngs>;       //! container of TrackCoefVtx coefficients at single vertex cand.
  using ArrTrAux     = std::array<TrcAuxPar,MaxProngs>;          //! container of Track aux params ...
  using AttTrDeriv   = std::array<TrackDeriv2,MaxProngs>;        //! container of Track derivatives
  
  // Collection of objects related to particular vertex candidate
  struct CandSet {
    ArrTrack     tracks;
    ArrTrackCovI covs;
    ArrTrCoef    trCoefs;
    AttTrDeriv   trDeriv;
    Ch2iDerivatives chi2deriv;
  };
